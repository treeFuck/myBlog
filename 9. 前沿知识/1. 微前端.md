...
### 3.1 微应用的切换
采用路由分发策略，将不同的微应用与不同的路由关联起来，一旦浏览器的 url 发送变化，就会触发 qiankun 的匹配逻辑，根据 activeRule 匹配到的微应用就会被插入到指定的 container 中。

路由匹配机制、微应用资源挂载卸载机制还没搞懂。

这部分功能，主要基于 single-spa 的路由劫持。
...
```
对于不支持 proxy 的低版本浏览器，提出了 SnapshotSandbox 模式沙箱。 
...
### 3.2.1 css 隔离
1. 约定式编程，给 css 编写定义一定的约束，比如不同的微应用使用不同的 class 前缀。
